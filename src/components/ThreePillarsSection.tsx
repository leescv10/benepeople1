...
export default function ThreePillarsSection({ config }: ThreePillarsSectionProps) {
  ...
  const p2Title = config?.pillar2Title || "법무/노무 리스크 완전 배제";
  ...
  const p3Title = config?.pillar3Title || "원스톱 스마트 운영 시스템";
  const p3Sub = config?.pillar3Sub || "기업 커스텀 고도화 웹&앱 모니터링 ERP 무상 지원";
  const p3Val = config?.pillar3Val || "전용 근태 & 업무 ERP 무상 제공";
  const p3ValSub = config?.pillar3ValSub || "안면인식 · 고유 IP · GPS 연동";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <div className="bg-[#0D5C4E]/45 border border-white/10 rounded-2xl p-6 sm:p-8 flex flex-col justify-between shadow-xl relative overflow-hidden group hover:border-brand-accent/50 transition duration-300">
            ...
            <div>
              ...
              <h3 className="text-xl font-bold text-white mb-2">{p3Title}</h3>
              <p className="text-xs text-slate-350 font-medium">{p3Sub}</p>
            </div>
            ...
            <div className="mt-8 bg-[#073B31]/90 border border-blue-500/20 p-5 rounded-xl text-center flex flex-col items-center justify-center shadow-lg">
              ...
              <p className="text-sm font-bold text-white">{p3Val}</p>
              <span className="text-[10px] text-gray-400 mt-1">{p3ValSub}</span>
            </div>
          </div>
        </div>
      </div>
